...
class PyreNode(object):
    def __init__(self, ctx, pipe, outbox, *args, **kwargs):
        ...
        try:
            ...
            logging.warning(
                "can't set ROUTER_HANDOVER, needs zmq version >=4.1 but installed is {0}".format(
                    zmq.zmq_version()
                )
            )
        ...
        self.name = str(self.identity)[
            :6
        ]  # Our public name (default=first 6 uuid chars)
    ...
    def start(self):
        ...
        if self.beacon_port:
            ...
            if self.port < 0:
                ...
                logging.critical(
                    "Random port assignment for incoming messages failed. Exiting."
                )
                sys.exit(-1)
            else:
                self.bound = True
    ...
    def join_peer_group(self, peer, groupname):
        ...
        logger.debug(
            "({0}) JOIN name={1} group={2}".format(
                self.name, peer.get_name(), groupname
            )
        )
        return grp
    ...
    def recv_peer(self):
        ...
        if peer.messages_lost(zmsg):
            logger.warning(
                "{0} messages lost from {1}".format(self.identity, peer.identity)
            )
            self.remove_peer(peer)
            return
        ...
        if zmsg.id == ZreMsg.HELLO:
            ...
            logger.debug(
                "({0}) ENTER name={1} endpoint={2}".format(
                    self.name, peer.get_name(), peer.get_endpoint()
                )
            )
    ...
    def ping_peer(self, peer_id):
        ...
        if time.time() > peer.expired_at:
            logger.debug(
                "({0}) peer expired name={1} endpoint={2}".format(
                    self.name, peer.get_name(), peer.get_endpoint()
                )
            )
            self.remove_peer(peer)
            ...
            # for peer management.
            logger.debug(
                "({0}) peer seems dead/slow name={1} endpoint={2}".format(
                    self.name, peer.get_name(), peer.get_endpoint()
                )
            )
