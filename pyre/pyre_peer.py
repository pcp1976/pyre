...
class PyrePeer(object):
    ...
    def connect(self, reply_to, endpoint):
        ...
        logger.debug(
            "Connecting to peer {0} on endpoint {1}".format(self.identity, endpoint)
        )
    ...
    def send(self, msg):
        if self.connected:
            ...
            try:
                ...
                self.disconnect()
                logger.debug(
                    "{0} Error while sending {1} to peer={2} sequence={3}".format(
                        self.origin, msg.get_command(), self.name, msg.get_sequence()
                    )
                )
                return -1

            logger.debug(
                "{0} send {1} to peer={2} sequence={3}".format(
                    self.origin, msg.get_command(), self.name, msg.get_sequence()
                )
            )
    ...
    def messages_lost(self, msg):
        ...
        logger.debug(
            "(%s) recv %s from peer=%s sequence=%d"
            % (self.origin, msg.get_command(), self.name, msg.get_sequence())
        )
        ...
        if self.want_sequence != msg.get_sequence():
            logger.debug(
                "(%s) seq error from peer=%s expect=%d, got=%d",
                self.origin,
                self.name,
                self.want_sequence,
                msg.get_sequence(),
